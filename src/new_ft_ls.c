...
/*   Updated: 2016/02/25 15:09:24 by udelorme         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int			open_file(char *path, t_dir_content **dirs)
{
	struct stat	file;
	t_dir_item		*prop;
	int				lstat_ret;
	...
	if ((lstat_ret = lstat(path, &file)) == 0)
	{
		...
		t_item_push(&prop, t_item_new(NULL, path));
		print_ls(first->items, params);
		return (1);
	}
	return (0);
...
static t_dir_content			*open_dirs(char **paths, char *params)
{
	...
	while (paths[++i])
	{
		...
		else if (open_file(paths[i], &files))
			ft_trace(NULL, "debug");
		else
